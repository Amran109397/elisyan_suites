...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (spaBookingForm) {
        // Treatment selection
        const selectTreatmentButtons = document.querySelectorAll('.select-treatment');
        
        selectTreatmentButtons.forEach(button => {
            button.addEventListener('click', function() {
                // Remove active class from all treatment cards
                document.querySelectorAll('.treatment-card').forEach(card => {
                    card.classList.remove('active');
                });
                
                // Add active class to selected treatment card
                this.closest('.treatment-card').classList.add('active');
                
                // Update booking summary
                const treatment = this.getAttribute('data-treatment');
                const treatmentPrice = this.closest('.treatment-card').querySelector('.price').textContent;
                
                document.getElementById('selected-treatment').innerHTML = `
                    <h6>${treatment}</h6>
                    <p>${treatmentPrice}</p>
                `;
                
                document.getElementById('selected-treatment').style.display = 'block';
                document.getElementById('total-price').textContent = treatmentPrice;
        ...
        timeSlots.forEach(slot => {
            slot.addEventListener('click', function() {
                ...
                document.getElementById('selected-datetime').style.display = 'block';
                document.getElementById('datetime-display').textContent = `${selectedDate} at ${selectedTime}`;
            });
        });
        ...
        therapistCards.forEach(card => {
            card.addEventListener('click', function() {
                // Remove selected class from all therapist cards
                therapistCards.forEach(c => c.classList.remove('selected'));
                
                // Add selected class to clicked therapist card
                this.classList.add('selected');
                
                // Update booking summary
                const therapistName = this.querySelector('h6').textContent;
                
                document.getElementById('selected-therapist').style.display = 'block';
                document.getElementById('therapist-display').textContent = therapistName;
            });
        });
        
        // Skip therapist selection
        const skipTherapistBtn = document.getElementById('skip-therapist');
        
        if (skipTherapistBtn) {
            skipTherapistBtn.addEventListener('click', function() {
                // Move to next step
                const currentStep = this.closest('.booking-step');
                const nextStep = currentStep.nextElementSibling;
        ...
        if (confirmBookingBtn) {
            confirmBookingBtn.addEventListener('click', function() {
                ...
                setTimeout(() => {
                    alert('Spa booking confirmed! You will receive a confirmation email shortly.');
    ...
    if (restaurantBookingForm) {
        restaurantBookingForm.addEventListener('submit', function(e) {
            e.preventDefault();
            ...
            if (!customerName || !customerEmail || !customerPhone || !bookingDate || !bookingTime || !guests) {
                alert('Please fill in all required fields');
                return;
            }
            ...
            // Simulate booking confirmation
            setTimeout(() => {
                alert('Restaurant booking confirmed! You will receive a confirmation email shortly.');
    ...
    if (eventBookingForm) {
        // Venue selection
        const venueCards = document.querySelectorAll('.venue-option');
        
        venueCards.forEach(card => {
            card.addEventListener('click', function() {
                // Remove active class from all venue cards
                venueCards.forEach(c => c.classList.remove('active'));
                
                // Add active class to clicked venue card
                this.classList.add('active');
                
                // Update booking summary
                const venueName = this.getAttribute('data-venue');
                const venuePrice = this.getAttribute('data-price');
                ...
                document.getElementById('event-summary').innerHTML = `
                    ...
                    <p><strong>Price:</strong> $${venuePrice}</p>
                `;
                document.getElementById('total-price').textContent = `$${venuePrice}`;
            });
        });
        ...
        eventBookingForm.addEventListener('submit', function(e) {
            ...
            if (!venueName || !eventDate || !guestCount || !eventType || !customerName || !customerEmail || !customerPhone) {
                alert('Please fill in all required fields');
                return;
            }
            ...
            // Simulate booking confirmation
            setTimeout(() => {
                alert('Event booking confirmed! Our event coordinator will contact you shortly.');
                window.location.href = 'booking-confirmation.html';
            }, 1500);
        });
    }
    ...
    if (paymentForm) {
        ...
        paymentMethods.forEach(method => {
            method.addEventListener('click', function() {
                ...
                document.getElementById('paypal-details-form').style.display = 'none';
                document.getElementById('bank-transfer-details-form').style.display = 'none';
                
                // Show selected payment form
                if (selectedMethod === 'credit-card') {
                    document.getElementById('card-details-form').style.display = 'block';
                } else if (selectedMethod === 'paypal') {
                    document.getElementById('paypal-details-form').style.display = 'block';
                } else if (selectedMethod === 'bank-transfer') {
                    document.getElementById('bank-transfer-details-form').style.display = 'block';
                }
            });
        });
    ...
    galleryImages.forEach(img => {
        ...
        img.addEventListener('error', function() {
            ...
            this.src = 'https://images.unsplash.com/photo-1571896349842-33c89424de2d?ixlib=rb-4.0.3&auto=format&fit=crop&w=500&q=80';
