            ...
            <div class="card">
                <div class="card-header bg-warning">
                    <h3 class="card-title text-white">
                        <i class="fas fa-edit"></i> Edit Room: {{ $room->room_number }}
                    </h3>
                ...
                <div class="card-body">
                    ...
                    <form action="{{ route('rooms.update', $room->id) }}" method="POST" id="roomForm">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" 
                                        ...
                                        <option value="out_of_service" {{ old('status', $room->status) == 'out_of_service' ? 'selected' : '' }}>
                                            Out of Service
                                        </option>
                                        <option value="blocked" {{ old('status', $room->status) == 'blocked' ? 'selected' : '' }}>
                                            Blocked
                                        </option>
                                        <option value="renovation" {{ old('status', $room->status) == 'renovation' ? 'selected' : '' }}>
                                            Renovation
                                        </option>
                                    </select>
                                    @error('status')
                        ...
                        <div class="form-group mt-4">
                            ...
                            <a href="{{ route('rooms.show', $room->id) }}" class="btn btn-info btn-lg">
...
<script>
    $(document).ready(function() {
        ...
        $('#roomForm').on('submit', function() {
            $('.btn-warning').prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Updating...');
        });
    });
</script>
@endsection