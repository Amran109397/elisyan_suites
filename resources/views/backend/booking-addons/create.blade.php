        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Create Booking Addon</h3>
                ...
                <div class="card-body">
                    <form action="{{ route('booking-addons.store') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('addon_id') is-invalid @enderror" id="addon_id" name="addon_id" required>
                                        ...
                                        @foreach($addons as $addon)
                                            <option value="{{ $addon->id }}">{{ $addon->name }} ({{ number_format($addon->price, 2) }})</option>
                                        @endforeach
                                    </select>
                                    @error('addon_id')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
