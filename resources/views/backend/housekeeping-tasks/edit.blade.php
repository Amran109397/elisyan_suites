        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Edit Housekeeping Task</h3>
                ...
                <div class="card-body">
                    <form action="{{ route('housekeeping-tasks.update', $task->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('property_id') is-invalid @enderror" id="property_id" name="property_id" required>
                                        ...
                                        @foreach($properties as $property)
                                            <option value="{{ $property->id }}" {{ old('property_id', $task->property_id) == $property->id ? 'selected' : '' }}>{{ $property->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('property_id')
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('room_id') is-invalid @enderror" id="room_id" name="room_id" required>
                                        ...
                                        @foreach($rooms as $room)
                                            <option value="{{ $room->id }}" {{ old('room_id', $task->room_id) == $room->id ? 'selected' : '' }}>{{ $room->room_number }}</option>
                                        @endforeach
                                    </select>
                                    @error('room_id')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-select @error('staff_id') is-invalid @enderror" id="staff_id" name="staff_id">
                                        ...
                                        @foreach($staff as $member)
                                            <option value="{{ $member->id }}" {{ old('staff_id', $task->staff_id) == $member->id ? 'selected' : '' }}>{{ $member->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('staff_id')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local" class="form-control @error('completed_at') is-invalid @enderror" id="completed_at" name="completed_at" value="{{ old('completed_at', $task->completed_at ? $task->completed_at->format('Y-m-d\TH:i') : '') }}">
                                    @error('completed_at')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                    @enderror
                                </div>
